namespace LuaNET.LuaJIT
{
	...
	public struct lua_State : IEquatable<lua_State>
	{
		...
		public static bool operator ==(lua_State state1, lua_State state2) =>
			state1.Handle == state2.Handle;
		...
		public override readonly bool Equals(object? other) =>
			other is lua_State state && Equals(state);
	...
	public static class Lua
	{
		...
		public static unsafe luaL_Reg AsLuaLReg(
			string name,
			delegate* unmanaged[Cdecl]<lua_State, int> func
		) => new() { name = name, func = (nint)func };
		...
		public static extern void luaJIT_profile_start(
			lua_State L,
			string mode,
		...
		private static extern nint _luaJIT_profile_dumpstack(
			lua_State L,
			string fmt,
			int depth,
			ref size_t len
		...
		public static string? luaJIT_profile_dumpstack(
			lua_State L,
			string fmt,
			int depth,
			ref size_t len
		...
		public const string LUA_PATH_CONFIG =
			LUA_DIRSEP
			+ "\n"
			+ LUA_PATHSEP
			+ "\n"
		...
		{
			var panic = _lua_atpanic(
				L,
				panicf == null ? 0 : Marshal.GetFunctionPointerForDelegate(panicf)
			);
		...
		[DllImport(DllName, CallingConvention = Convention, EntryPoint = "lua_tolstring")]
		public static extern unsafe byte* lua_tolstring_ptr(lua_State L, int idx, ref ulong len);
		...
		{
			var ret = _lua_tocfunction(L, idx);
		...
		[DllImport(DllName, CallingConvention = Convention, EntryPoint = "lua_getfield")]
		public static extern unsafe void lua_getfield_ptr(lua_State L, int idx, byte* k);
		...
		[DllImport(DllName, CallingConvention = Convention, EntryPoint = "lua_setfield")]
		public static extern unsafe void lua_setfield_ptr(lua_State L, int idx, byte* k);
		...
		public static int lua_load(lua_State L, lua_Reader? reader, nuint dt, string chunkname)
		{
			return _lua_load(
				L,
				reader == null ? 0 : Marshal.GetFunctionPointerForDelegate(reader),
				dt,
				chunkname
			);
		}
		...
		public static int lua_sethook(lua_State L, lua_Hook? func, int mask, int count)
		{
			return _lua_sethook(
				L,
				func == null ? 0 : Marshal.GetFunctionPointerForDelegate(func),
				mask,
				count
			);
		}
		...
		private static extern int _lua_loadx(
			lua_State L,
			nint reader,
			nuint dt,
			string chunkname,
